  ...
  class getTestName: SyntaxVisitor {
    ...
    override func visit(_ node: FunctionDeclSyntax) -> SyntaxVisitorContinueKind {
      ...
      if bodyCheck != nil {
        ...
        body = String(body[body.index(after: body.startIndex)..<body.index(before: body.endIndex)])
          .trimmingCharacters(in: charactersToRemove)
      ...
      if testCase.status == "fail" {
        ...
        for (rowIdx, row) in contextLines.enumerated() {
          if found {
            ...
            found = true
            if let endRange = row.range(
              of: "Test Case ", options: [], range: (startRange.lowerBound..<row.endIndex))
            {
              let sum = row.distance(from: row.startIndex, to: startRange.lowerBound)
              let lowerBound1 = String.Index(
                encodedOffset: sum
                  + row.distance(from: startRange.lowerBound, to: endRange.lowerBound))
              ...
              if row.contains(testCase.name) && row[startIndex] != "[" {
                ...
                if let startRange = row.range(of: "started at", options: .backwards) {
                  if let newStartRange = row.range(
                    of: ".", options: [], range: (startRange.upperBound..<row.endIndex))
                  {
                    let newIndex = row.index(newStartRange.upperBound, offsetBy: 3)
                    let sum = row.distance(from: row.startIndex, to: newIndex)
                    if let endRange = row.range(
                      of: "Test Case ", options: [],
                      range: (newStartRange.upperBound..<row.endIndex))
                    {
                      let lowerBound1 = String.Index(
                        encodedOffset: sum + row.distance(from: newIndex, to: endRange.lowerBound))
                      let upperBound1 = String.Index(encodedOffset: sum)
                      let newSubstring = row[upperBound1..<lowerBound1]
                      xmlTests[testIdx].message = String(newSubstring)
                      break
                    } else if let endRange = row.range(
                      of: "Current stack trace:", options: [],
                      range: (newStartRange.upperBound..<row.endIndex))
                    {
                      let lowerBound1 = String.Index(
                        encodedOffset: sum + row.distance(from: newIndex, to: endRange.lowerBound))
        ...
        for (rowIdx, row) in contextLinesOutput.enumerated() {
          ...
          if row.count > 0 {
              ...
              && !row.contains("error")
            {
              ...
              && !row.contains("error")) || row.hasPrefix("\\")
            {
              if start < rowIdx {
                xmlTests[testIdx].output =
